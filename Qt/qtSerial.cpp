...
int QtSerial::openPort(char *dev_name, int baudrate)
{
	...
	if(spd != -1)
	{
		...
		{
			emit message(QString("<font color=\"#FF0000\">ERROR setting serial port input speed at QtSerial::openPort!</font>"));
			return -1;
		}

		if (cfsetospeed(&options, (speed_t) spd) != 0)
		{
			emit message(QString("<font color=\"#FF0000\">ERROR setting serial port output speed at QtSerial::openPort!</font>"));
			return -1;
		}
	}
	else
	{
		emit message(QString("<font color=\"#FF0000\">ERROR: Wrong value for speed parameter at QtSerial::openPort!</font>"));
		return -1;
	}
	...
	{
		emit message(QString("<font color=\"#FF0000\">ERROR fluhsing serial input buffer at QtSerial::openPort!</font>"));
		return -1;
	}
	...
	{
		emit message(QString("<font color=\"#FF0000\">ERROR setting serial port attributes at QtSerial::openPort!</font>"));
		return -1;
	}
...
int QtSerial::writeData(unsigned char *c, QString callingClassName)
{
	int n = write(mDev_fd, c, 1);

	if (n < 0)
	{
		emit message(QString("<font color=\"#FF0000\">ERROR '%1=%2' when writing to serial device at QtSerial::writeData called from %3.</font>").arg(errno).arg(strerror(errno)).arg(callingClassName));
...
int QtSerial::readData(unsigned char *buf, int nChars, QString callingClassName)
{
	...
	while (nChars > 0)
	{
		...
		if (returnValue == -1)
		{
			emit message(QString("<font color=\"#FF0000\">ERROR '%1=%2' <br>when selecting serial device at QtSerial::readData called from %3.</font>").arg(errno).arg(strerror(errno)).arg(callingClassName));
			return errno;
		}
		else
		{
			...
			{
				// timeout
//				emit message(QString("<font color=\"#FF0000\">ERROR No data available within %1 µs when using select() on serial device at QtSerial::readData called from %2.</font>").arg(READ_TIMEOUT_ATMEL).arg(callingClassName));
//				emit message(QString("<font color=\"#FF0000\">ERROR %1: %2.</font>").arg(errno).arg(strerror(errno)));
				emit message("(ignored timeout, Arduino needs some time)");
		...
		{
			emit message(QString("<font color=\"#FF0000\">ERROR '%1=%2' when using read() on serial device at QtSerial::readData called from %3.</font>").arg(errno).arg(strerror(errno)).arg(callingClassName));
